...
namespace CarePlatform.Data.VistA
{
    public class VistaUtils
    {
        ...
        {
            if (response != "" && !Regex.IsMatch(response, @"^\d+$"))
            {
        ...
        public static string setDirectionParam(string direction)
        {
            ...
            {
                throw new ArgumentException( "Invalid direction.  Must be 1 or -1.");
            }
            return direction;
        }
    ...
    public class StringUtils
    {
        ...
        {
            string[] flds = s.Split(new string[]{ delimiter}, StringSplitOptions.None);
        ...
        public static string StringToSearchString(string SearchString)
        {
            if (string.IsNullOrEmpty(SearchString) ||
                (SearchString.ToUpper().Last() == 'A' && SearchString.Length <= 1))
            {
                SearchString = "";
            }
            else
            {
                if (SearchString.ToUpper().Last() == 'A')
                {
                    SearchString = SearchString.Substring(0, SearchString.Length - 1);
                }

                byte[] tempbytes = Encoding.ASCII.GetBytes(SearchString.Trim());
                tempbytes[tempbytes.Length - 1]--;
                string EncodedSearchString = Encoding.ASCII.GetString(tempbytes);
                SearchString = EncodedSearchString.ToUpper() + "~";
            }
            return SearchString;
        }
